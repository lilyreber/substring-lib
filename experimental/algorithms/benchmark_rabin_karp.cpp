...
static void BM_Rabin_Karp_Cold(benchmark::State &state) {
    std::string text = "hello world, this is a test string";
    std::string pattern = "test";
    for (auto _ : state) {
        benchmark::DoNotOptimize(search_pattern(text, pattern, AlgorithmType::RABIN_KARP));
    }
}

BENCHMARK(BM_Rabin_Karp_Cold);

// Benchmark: search pattern in large text (100MB, cold)
static void BM_Rabin_Karp_LargeText_Cold(benchmark::State &state) {
...
BENCHMARK(BM_Rabin_Karp_LargeText_Cold);

// Benchmark: search pattern in large text (100MB, hot)
static void BM_Rabin_Karp_Hot(benchmark::State &state) {
...
BENCHMARK(BM_Rabin_Karp_Hot);

// Benchmark: speed per mb
static void BM_Rabin_Karp_Per_MB(benchmark::State &state) {
...
BENCHMARK(BM_Rabin_Karp_Per_MB)->Range(1024 * 1024, 100 * 1024 * 1024);